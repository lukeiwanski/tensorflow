...
class MaxPoolingGradOp<SYCLDevice, T> : public OpKernel {
 public:
  explicit MaxPoolingGradOp(OpKernelConstruction* context)
      : OpKernel(context) {
    string data_format;
    OP_REQUIRES_OK(context, context->GetAttr("data_format", &data_format));
    OP_REQUIRES(context, FormatFromString(data_format, &data_format_),
...
class MaxPoolGradSYCL {
 ...
 public:
  MaxPoolGradSYCL(const int depth, const int batch,
      ...
      : p_(depth, batch, in_rows, in_cols, output_shape, window,
           stride, padding),
  ...
  void operator()(cl::sycl::item<1> item) {
    ...
    if (maxidx != -1) {
      SyclAtomicAdd(
          output_backprop +
              n * p_.in_rows_ * p_.in_cols_ * p_.depth_ +
              maxidx,
          input_backprop[index]);
    }else{
      output_backprop[index/p_.out_rows_*p_.in_rows_+index%p_.out_rows_] = input_backprop[index];
    }
  }
  ...
  static void launch(OpKernelContext* context, const Tensor& tensor_in,
    ...
    device.sycl_queue().submit([&](cl::sycl::handler& cgh) {
      ...
      MaxPoolGradSYCL<T> max_pool(
          depth, batch, in_rows, in_cols, out, window, stride,
...
class MaxPoolGradGradSYCL {
 ...
 public:
  ...
  void operator()(cl::sycl::item<1> item) {
    ...
    if (maxidx != -1) {
      output_backprop[index] = input_backprop[n * p_.in_rows_ * p_.in_cols_
                                                * p_.depth_ + maxidx];
    }
  }
